...
havoc_stage:
  ...
  /* ACO: 定义局部变量用于记录算子序列 */
  u32 aco_op_seq[ACO_MAX_STACK];
  u32 aco_seq_len = 0;
  u32 aco_prev_op = ACO_START_NODE;
  ...
  for (stage_cur = 0; stage_cur < stage_max; stage_cur++) {
    ...
    /* ======================================================
       分支 1: 启用 ACO 变异调度 (Ours / AFL-ACO)
       ====================================================== */
    if (use_aco_mutation) {
      
      /* ACO: 重置本次 stacking 的状态 */
      aco_seq_len = 0;
      aco_prev_op = ACO_START_NODE;

      /* 记录当前路径数量，用于判断是否发现了新路径 (Reward Calculation) */
      u64 saved_queued_paths = queued_paths;

      for (i = 0; i < use_stacking; i++) {

        /* ACO: 选择算子 */
        u32 aco_op = ACO_SelectOperator(aco_prev_op);

        /* ACO: 记录序列 */
        if (aco_seq_len < ACO_MAX_STACK) {
          aco_op_seq[aco_seq_len++] = aco_op;
        }
        aco_prev_op = aco_op;
        ...
        switch (aco_op) {
          ...
          case ACO_OP_FLIP_8:
            ...
            break;
          case ACO_OP_FLIP_16:
            if (temp_len >= 2) *(u16*)(out_buf + UR(temp_len - 1)) ^= 0xFFFF;
            break;
          case ACO_OP_FLIP_32:
            if (temp_len >= 4) *(u32*)(out_buf + UR(temp_len - 3)) ^= 0xFFFFFFFF;
            break;
          case ACO_OP_INTEREST_8:
            out_buf[UR(temp_len)] = interesting_8[UR(sizeof(interesting_8))];
            break;
          case ACO_OP_INTEREST_16:
            if (temp_len < 2) break;
            if (UR(2)) *(u16*)(out_buf + UR(temp_len - 1)) = interesting_16[UR(sizeof(interesting_16) >> 1)];
            else       *(u16*)(out_buf + UR(temp_len - 1)) = SWAP16(interesting_16[UR(sizeof(interesting_16) >> 1)]);
            break;
          case ACO_OP_INTEREST_32:
            if (temp_len < 4) break;
            if (UR(2)) *(u32*)(out_buf + UR(temp_len - 3)) = interesting_32[UR(sizeof(interesting_32) >> 2)];
            else       *(u32*)(out_buf + UR(temp_len - 3)) = SWAP32(interesting_32[UR(sizeof(interesting_32) >> 2)]);
            break;
          case ACO_OP_ARITH_8:
            if (UR(2)) out_buf[UR(temp_len)] += 1 + UR(ARITH_MAX);
            else       out_buf[UR(temp_len)] -= 1 + UR(ARITH_MAX);
            break;
          case ACO_OP_ARITH_16:
            if (temp_len < 2) break;
            {
              u32 pos = UR(temp_len - 1);
              u16 num = 1 + UR(ARITH_MAX);
              if (UR(2)) { if (UR(2)) *(u16*)(out_buf + pos) += num; else *(u16*)(out_buf + pos) = SWAP16(SWAP16(*(u16*)(out_buf + pos)) + num); }
              else       { if (UR(2)) *(u16*)(out_buf + pos) -= num; else *(u16*)(out_buf + pos) = SWAP16(SWAP16(*(u16*)(out_buf + pos)) - num); }
            }
            break;
          case ACO_OP_ARITH_32:
            if (temp_len < 4) break;
            {
              u32 pos = UR(temp_len - 3);
              u32 num = 1 + UR(ARITH_MAX);
              if (UR(2)) { if (UR(2)) *(u32*)(out_buf + pos) += num; else *(u32*)(out_buf + pos) = SWAP32(SWAP32(*(u32*)(out_buf + pos)) + num); }
              else       { if (UR(2)) *(u32*)(out_buf + pos) -= num; else *(u32*)(out_buf + pos) = SWAP32(SWAP32(*(u32*)(out_buf + pos)) - num); }
            }
            break;
          case ACO_OP_EXTRAS:
            if (!extras_cnt && !a_extras_cnt) break;
            if (!extras_cnt || (a_extras_cnt && UR(2))) {
               u32 use_extra = UR(a_extras_cnt);
               u32 extra_len = a_extras[use_extra].len;
               if (extra_len > temp_len) break;
               u32 insert_at = UR(temp_len - extra_len + 1);
          ...
          case ACO_OP_BLK_DEL:
            if (temp_len < 2) break;
            {
              u32 del_len = choose_block_len(temp_len - 1);
              u32 del_from = UR(temp_len - del_len + 1);
          ...
          case ACO_OP_BLK_INS:
            if (temp_len + HAVOC_BLK_XL < MAX_FILE) {
               ...
               u8* new_buf;
               if (actually_clone) { clone_len = choose_block_len(temp_len); clone_from = UR(temp_len - clone_len + 1); }
               else                { clone_len = choose_block_len(HAVOC_BLK_XL); clone_from = 0; }
               clone_to = UR(temp_len);
               new_buf = ck_alloc_nozero(temp_len + clone_len);
               memcpy(new_buf, out_buf, clone_to);
               if (actually_clone) memcpy(new_buf + clone_to, out_buf + clone_from, clone_len);
               else                memset(new_buf + clone_to, UR(2) ? UR(256) : out_buf[UR(temp_len)], clone_len);
            ...
            break;
          case ACO_OP_BLK_OVER:
            if (temp_len < 2) break;
            {
               u32 copy_len = choose_block_len(temp_len - 1);
               u32 copy_from = UR(temp_len - copy_len + 1);
               u32 copy_to = UR(temp_len - copy_len + 1);
               if (UR(4)) { if (copy_from != copy_to) memmove(out_buf + copy_to, out_buf + copy_from, copy_len); }
      ...
      /* ACO 反馈 */
      u8 reward_level = 0;
      if (queued_paths > saved_queued_paths) {
          reward_level = 2;
      }
      ACO_NotifyFeedback(aco_op_seq, aco_seq_len, reward_level);

    } else {
      /* ======================================================
         分支 2: 原版 AFL 随机调度 (Baseline)
         ====================================================== */
      ...
      for (i = 0; i < use_stacking; i++) {
        ...
        switch (UR(15 + ((extras_cnt + a_extras_cnt) ? 2 : 0))) {
          ...
          case 6:
            if (temp_len < 2) break;
            if (UR(2)) { u32 pos = UR(temp_len - 1); *(u16*)(out_buf + pos) -= 1 + UR(ARITH_MAX); }
            else       { u32 pos = UR(temp_len - 1); u16 num = 1 + UR(ARITH_MAX); *(u16*)(out_buf + pos) = SWAP16(SWAP16(*(u16*)(out_buf + pos)) - num); }
            break;
          case 7:
            if (temp_len < 2) break;
            if (UR(2)) { u32 pos = UR(temp_len - 1); *(u16*)(out_buf + pos) += 1 + UR(ARITH_MAX); }
          ...
          case 8:
            if (temp_len < 4) break;
            if (UR(2)) { u32 pos = UR(temp_len - 3); *(u32*)(out_buf + pos) -= 1 + UR(ARITH_MAX); }
            else       { u32 pos = UR(temp_len - 3); u32 num = 1 + UR(ARITH_MAX); *(u32*)(out_buf + pos) = SWAP32(SWAP32(*(u32*)(out_buf + pos)) - num); }
            break;
          case 9:
            if (temp_len < 4) break;
            if (UR(2)) { u32 pos = UR(temp_len - 3); *(u32*)(out_buf + pos) += 1 + UR(ARITH_MAX); }
          ...
          case 16: {
              ...
              u8* new_buf;
              if (!extras_cnt || (a_extras_cnt && UR(2))) {
                use_extra = UR(a_extras_cnt); extra_len = a_extras[use_extra].len;
              } else {
                use_extra = UR(extras_cnt); extra_len = extras[use_extra].len;
              }
              if (temp_len + extra_len >= MAX_FILE) break;
              new_buf = ck_alloc_nozero(temp_len + extra_len);
              memcpy(new_buf, out_buf, insert_at);
              if (!extras_cnt || (a_extras_cnt && UR(2))) memcpy(new_buf + insert_at, a_extras[use_extra].data, extra_len);
              else                                        memcpy(new_buf + insert_at, extras[use_extra].data, extra_len);
      ...
      } // End Stack Loop (Original)

      /* 执行测试 (Baseline 分支) - 必须在 stack 循环外面！ */
      if (common_fuzz_stuff(argv, out_buf, temp_len)) goto abandon_entry;
...
int main(int argc, char** argv) {
  ...
  check_asan_opts();
    ...
    /* ======================================================== */
    /* [Ablation Study] 消融实验控制逻辑                          */
    /* ======================================================== */

    /* 1. 控制 ACO 变异调度 (AFL_NO_ACO) */
    if (getenv("AFL_NO_ACO")) {
        use_aco_mutation = 0;
        OKF("Ablation: ACO Mutation Scheduling DISABLED (AFL_NO_ACO set).");
    } else {
        use_aco_mutation = 1;
        ACO_init(); /* 初始化 ACO 矩阵 */
        OKF("Ablation: ACO Mutation Scheduling ENABLED.");
    }

    /* 2. 控制 MLP 种子调度 (AFL_NO_MLP) */
    if (getenv("AFL_NO_MLP")) {
        use_nn_scheduling = 0;
        OKF("Ablation: MLP Seed Scheduling DISABLED (AFL_NO_MLP set).");
    } else {
        /* 尝试开启 MLP */
        use_nn_scheduling = 1;
        ...
        if (redis_connected) {
            ...
            OKF("Ablation: MLP Seed Scheduling ENABLED.");
        } else {
            /* Redis 连接失败，强制回退 */
            use_nn_scheduling = 0;
            WARNF("Redis connection failed. MLP Scheduling disabled.");
        }
    }
    /* ======================================================== */
